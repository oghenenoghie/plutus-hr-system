...
class LedgerStatementLine(Base):
    ...
    Named Ledger* (and kept at a distinct table from bank_statement_lines)
    because this is a separate, org-wide-by-account-code reconciliation
    feature from CompanyBankAccount's own per-bank-account statement lines
    (see app/models/bank_statement_line.py) — the two were built
    independently and happen to overlap in scope, not in schema.
    """

    __tablename__ = "ledger_statement_lines"
