...
from app.services.employee_photos import purge_expired_employee_photos
...
def purge_expired_employee_photos_for_all_orgs() -> None:
    today = datetime.now(UTC).date()

    def _run_one(org_id: uuid.UUID) -> None:
        with system_session(org_id) as db:
            purge_expired_employee_photos(db, org_id, as_of=today)

    _for_every_org("employee-photo-retention", _run_one)
...
def build_scheduler() -> BackgroundScheduler:
    ...
    scheduler.add_job(
        purge_expired_employee_photos_for_all_orgs,
        CronTrigger(hour=2, minute=15),
        id="employee-photo-retention",
        max_instances=1,
        coalesce=True,
        misfire_grace_time=3600,
    )
